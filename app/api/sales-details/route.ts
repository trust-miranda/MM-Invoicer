...
sales_by_week AS (
    ...
    LEFT JOIN ft2 WITH (NOLOCK) ON ft.ftstamp=ft2.ft2stamp
    WHERE ft.no IN (9,12,111,252,325,394,477,1413,3742)
    AND ft.tipodoc IN (1,3)
    AND ft2.u_ftcir IN ('0','1')
    AND ft.anulado=0
	  AND ft.nome2 >''
    AND ft.fdata > DATEFROMPARTS(YEAR(GETDATE()), 1, 1)
    GROUP BY ft.no, ft.nome2, ft2.u_ftcir, DATEADD(WEEK, DATEDIFF(WEEK, 0, ft.fdata), 0)
),
...
SELECT 
    aw.week_start,
    CASE 
        WHEN sbw.nome2 = 'Liberty' THEN 'Generali'
        ELSE sbw.nome2
    END AS nome2,
	sbw.u_ftcir,
    DENSE_RANK() OVER (ORDER BY aw.week_start ASC) AS week_number,
    ISNULL(SUM(sbw.TotalFaturado), 0) AS total_sales_per_week_per_nome2_per_type,
...
AND nome2 IS NOT NULL
GROUP BY 
    aw.week_start, sbw.nome2, sbw.u_ftcir, ts.total_sales_per_week
ORDER BY 
    aw.week_start, sbw.nome2, sbw.u_ftcir
OPTION (MAXRECURSION 100);
  `;
    ...
    const resultWithFixedBigInt = result.map((entry: any) => ({
      ...
      u_ftcir: entry.u_ftcir === true ? "1" : "0", // Ensure consistency as string "0" or "1"
      week_number: Number(entry.week_number),
      total_sales_per_week_per_nome2_per_type:
        entry.total_sales_per_week_per_nome2_per_type,
    ...
    const totalCurrentWeek = currentWeekData.reduce(
      (sum: number, entry: any) =>
        sum + Number(entry.total_sales_per_week_per_nome2_per_type),
      0
    );
    ...
    return NextResponse.json(
      {
        ...
        currentWeekData: currentWeekData.map((entry: any) => ({
          nome2: entry.nome2,
          total_sales: entry.total_sales_per_week_per_nome2_per_type,
          u_ftcir: entry.u_ftcir,
        })),
        totalCurrentWeek,
