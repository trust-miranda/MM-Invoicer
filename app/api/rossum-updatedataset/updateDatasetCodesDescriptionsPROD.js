...
const fetchDataAndUpload = async () => {
  try {
    ...
    SELECT * FROM
        (SELECT 
            LTRIM(RTRIM(sc.refb)) as bundledCode, 
            LTRIM(RTRIM(st.ref)) as code, 
            LTRIM(RTRIM(st.design)) as description
        FROM st WITH (NOLOCK)
...
`);
    ...
    const uploadResponse = await axios.patch(
      "https://trust-saude.rossum.app/svc/master-data-hub/api/v1/dataset/PROD - codes_descriptions",
      data,
      {
        headers: {
