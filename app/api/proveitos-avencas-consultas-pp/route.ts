...
export async function GET(req: NextRequest) {
  try {
    console.log("Executing complex SQL query...");

    // Run the SQL query directly using $queryRaw with CTE and other advanced SQL structures.
...
WITH
    CTE AS (
        ...
        WHERE pd.processoFaturado = 0
			...
			AND pd.temRecaida=0
			AND am.codigo NOT IN ('01.00.00.82', '01.00.01.13','01.00.01.32')
...
SELECT
    CTE7.nome,
    SUM(CTE7.Final_ProveitoExpectavelAvenca) AS ProveitoExpectavelAvenca,
    SUM(CTE7.ProveitoExpectavelRenovacao) as ProveitoExpectavelRenovacao,
    SUM(CTE7.ProveitoExpectavelConsultas) as ProveitoExpectavelConsultas
FROM CTE7
WHERE nome LIKE '%TRUST%'
AND nome <> 'Lisboa - TRUST HCVP'
GROUP BY nome;
    `;

    // Convert BigInt fields to strings to avoid JSON serialization issues
    const processedResult = result.map((entry: any) =>
      Object.fromEntries(
    ...
    return NextResponse.json({ data: processedResult }, { headers });
  } catch (error) {
