    ...
    try {
      ...
      const records = await prisma.preDataset.findMany({
        ...
        select: {
          ...
          nrRequisicoes: true, // Include both fields for filtering
          nrRequisicoesRX: true,
        },
      });

      // Filter the records based on nrRequisicoes = nrRequisicoesRX
      const recordsFilteredByRequisicoes = records.filter(
        (record) => record.nrRequisicoes === record.nrRequisicoesRX
      );

      const processoEstadoClinicoIds = recordsFilteredByRequisicoes.map(
        (record) => record.idProcessoEstadoClinico
      );
      const processoEstadoSinistroIds = recordsFilteredByRequisicoes.map(
        (record) => record.idProcessoEstadoSinistro
      );
      const entidadeIds = recordsFilteredByRequisicoes.map(
        (record) => record.idEntidadeSeguradora
      );
      ...
      const combinedRecords = recordsFilteredByRequisicoes.map((record) => {
      ...
      const finalFilteredRecords = combinedRecords.filter((record) => {
        const dateDiff =
      ...
      const recordsWithStringifiedBigInt = finalFilteredRecords.map(
        (record: any) => {
